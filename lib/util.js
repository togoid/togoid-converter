...
export const mergePathStyle = (paths, fromId, toId, isRoute) => {
  // todo refactoring
  const toLabelId = toId.replace(/^from/, "label").replace(/^to/, "label");
  const toLabelPath = getPathStyle(fromId, toLabelId, isRoute);
  const fromLabelPath = getPathStyle(toLabelId, toId, isRoute);
  return paths.concat([toLabelPath, fromLabelPath]);
};
